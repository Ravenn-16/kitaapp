...
(() => {
  ...
  // src/parse.ts
  function readDcScriptSource(scriptEl, doc) {
    if (!scriptEl) return "";
    const inline = scriptEl.textContent || "";
    if (inline.trim()) return inline;
    const src = scriptEl.getAttribute("src");
    if (!src) return inline;
    try {
      const xhr = new XMLHttpRequest();
      xhr.open("GET", new URL(src, doc.baseURI || location.href).href, false);
      xhr.send(null);
      if (xhr.status === 0 || xhr.status >= 200 && xhr.status < 300) return xhr.responseText || "";
    } catch (e) {
      console.error("[dc-runtime] failed to load external data-dc script", src, e);
    }
    return inline;
  }
  function parseDcDocument(doc) {
    ...
    return {
      ...
      js: readDcScriptSource(scriptEl, doc),
      props,
      preview
    };
  }
